...
double integrand_in_nu_heat_integral(double nu, void * params){
    ...
    // HI
    species_sum = interp_fheat((nu - NUIONIZATION)/NU_over_EV, x_e) * hplank*(nu - NUIONIZATION) * f_H * (1-x_e) * HI_ion_crosssec(nu);

    // HeI
    species_sum += interp_fheat((nu - HeI_NUIONIZATION)/NU_over_EV, x_e) * hplank*(nu - HeI_NUIONIZATION) * f_He * (1-x_e) * HeI_ion_crosssec(nu);

    // HeII
    species_sum += interp_fheat((nu - HeII_NUIONIZATION)/NU_over_EV, x_e) * hplank*(nu - HeII_NUIONIZATION) * f_He * x_e * HeII_ion_crosssec(nu);
...
double integrand_in_nu_ion_integral(double nu, void * params){
    ...
    F_i = interp_nion_HI((nu - NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeI((nu - NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeII((nu - NUIONIZATION)/NU_over_EV, x_e) + 1;
    species_sum = F_i * f_H * (1-x_e) * HI_ion_crosssec(nu);

    // photoionization of HeI, prodicing e- of energy h*(nu - nu_HeI)
    F_i = interp_nion_HI((nu - HeI_NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeI((nu - HeI_NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeII((nu - HeI_NUIONIZATION)/NU_over_EV, x_e) + 1;
    species_sum += F_i * f_He * (1-x_e) * HeI_ion_crosssec(nu);

    // photoionization of HeII, prodicing e- of energy h*(nu - nu_HeII)
    F_i = interp_nion_HI((nu - HeII_NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeI((nu - HeII_NUIONIZATION)/NU_over_EV, x_e) + interp_nion_HeII((nu - HeII_NUIONIZATION)/NU_over_EV, x_e) + 1;
...
double integrand_in_nu_lya_integral(double nu, void * params){
    ...
    // HI
    species_sum = interp_n_Lya((nu - NUIONIZATION)/NU_over_EV, x_e) * f_H * (double)(1-x_e) * HI_ion_crosssec(nu);

    // HeI
    species_sum += interp_n_Lya((nu - HeI_NUIONIZATION)/NU_over_EV, x_e) * f_He * (double)(1-x_e) * HeI_ion_crosssec(nu);

    // HeII
    species_sum += interp_n_Lya((nu - HeII_NUIONIZATION)/NU_over_EV, x_e) * f_He * (double)x_e * HeII_ion_crosssec(nu);
...
double integrate_over_nu(double zp, double local_x_e, double lower_int_limit, int FLAG){
    ...
    */
    ...
    gsl_integration_qag (&F, lower_int_limit, 100*lower_int_limit, 0, rel_tol, 1000, GSL_INTEG_GAUSS61, w, &result, &error);
...
double kappa_10_elec(double T, int flag)
{
    ...
    if (T < TK[0]) { /* Use TK=1 K value if called at lower temperature */
        ...
        /* Power law extrapolation */
        ans  = kappa[KAPPA_10_elec_NPTS-1] + ((kappa[KAPPA_10_elec_NPTS-1] - kappa[KAPPA_10_elec_NPTS-2]) / (TK[KAPPA_10_elec_NPTS-1] - TK[KAPPA_10_elec_NPTS-2]) * (T-TK[KAPPA_10_elec_NPTS-1]));
    }
    else { /* Do spline */
...
double kappa_10_pH(double T, int flag)
{
    ...
    if (T < TK[0]){ /* Use TK=1 K value if called at lower temperature */
        ...
        /* Power law extrapolation */
        ans  = kappa[KAPPA_10_pH_NPTS-1] + ((kappa[KAPPA_10_pH_NPTS-1] - kappa[KAPPA_10_pH_NPTS-2]) / (TK[KAPPA_10_pH_NPTS-1] - TK[KAPPA_10_pH_NPTS-2]) * (T-TK[KAPPA_10_pH_NPTS-1]));
    } else { /* Do spline */
        ans = gsl_spline_eval (spline, T, acc);
    }
    ans = exp(ans);
