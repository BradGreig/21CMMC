    ...
    **Warning**: The :attr:`chain` member of this object has the shape:
    ...
    def _propose_stretch(self, p0, p1, lnprob0):
        ...
        * ``blob`` — The new meta data blobs or ``None`` if nothing was
        ...
        """ Want to modify the get_lnprob function as it obtains the log prob for all parameter sets including ones that would
            be thrown out when the are outside their respective bounds. It would be beneficial to include this step as the 
            likelihood computation is computationally expensive. It would remove redundant computations 
        """
